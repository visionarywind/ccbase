...
Node<K, V>::Node(const K k, const V v, int level) {
  ...
  // this->backward = new Node<K, V> *[level + 1];

  // Fill forward array with 0(NULL)
  memset(this->forward, 0, sizeof(Node<K, V> *) * (level + 1));
  // memset(this->backward, 0, sizeof(Node<K, V> *) * (level + 1));
};
...
int SkipList<K, V>::insert_element(const K key, const V value) {
  ...
  if (current == NULL || current->get_key() != key) {
    ...
    inserted_node->forward[0] = update[0]->forward[0];
    if (update[0]->forward[0] != NULL) update[0]->prev = inserted_node;
    update[0]->forward[0] = inserted_node;
    inserted_node->prev = update[0];
