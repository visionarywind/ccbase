...
bool SkipListAllocator::Free(void *addr) {
  ...
  if (it != total_block_.end()) {
    auto block = it->second;
    block->status_ = 0;
    if (block->stream_id_ != 0) {
      std::cout << "unknown stream id : " << std::endl;
    }
    auto &free_blocks = free_blocks_[block->stream_id_];
...
//////////////////
void *MultimapAllocator::Alloc(size_t size, uint32_t stream_id) {
  // std::cout << "alloc size : " << size << std::endl;
  size = align(size);
  std::lock_guard<std::mutex> locker(mutex_);
  auto &free_blocks = free_blocks_[stream_id];
  ...
  if (it == free_blocks.end()) {
    ...
    // std::cout << "malloc addr : " << addr << std::endl;
    block = new Block(addr, 1 << 30, 0);
    total_blocks_.emplace(addr, block);
  } else {
    block = it->second;
    // std::cout << "free_blocks erase : " << block->addr_ << std::endl;
    free_blocks.erase(it);
  ...
  if (remaining >= MIN_SPLIT_SIZE) {
    ...
    free_blocks.emplace(remaining_block->size_, remaining_block);
    // remaining_block->Print();
    // std::cout << "free_blocks insert : " << remaining_block->addr_ << std::endl;
    total_blocks_.emplace(remaining_block->addr_, remaining_block);
    block->size_ = size;
  }
  block->status_ = 1;
...
bool MultimapAllocator::Free(void *addr) {
  // std::cout << "free addr : " << addr << std::endl;
  std::lock_guard<std::mutex> locker(mutex_);
  auto it = total_blocks_.find(addr);
  if (it != total_blocks_.end()) {
    auto block = it->second;
    block->status_ = 0;
    // std::cout << "locate block : " << block << ", prev : " << block->prev_ << ", next : " << block->next_
    //           << ", stream_id : " << block->stream_id_ << std::endl;
    ...
    if (prev_block != nullptr) {
      ...
      if (prev_block->status_ == 0) {
        ...
        auto &&iter = free_blocks.equal_range(prev_block->size_);
        auto start = iter.first;
        auto end = iter.second;
        bool erase_flag = false;
        while (start != end) {
          if (start->second == prev_block) {
        ...
        total_blocks_.erase(prev_block->addr_);
        total_blocks_.erase(block->addr_);  // bugfix
        block->addr_ = prev_block->addr_;
        block->size_ += prev_block->size_;
        total_blocks_.emplace(block->addr_, block);
        // prev_block->Print();
        // std::cout << "remove prev : " << prev_block << std::endl;
        delete prev_block;
      } else {
    ...
    if (next_block != nullptr) {
      ...
      if (next_block->status_ == 0) {
        ...
        auto &&iter = free_blocks.equal_range(next_block->size_);
        auto start = iter.first;
        auto end = iter.second;
        bool erase_flag = false;
        while (start != end) {
          if (start->second == next_block) {
            free_blocks.erase(start);
            erase_flag = true;
            break;
          }
        ...
        total_blocks_.erase(next_block->addr_);
        // next_block->Print();
        // std::cout << "remove next : " << next_block << std::endl;
        delete next_block;
      } else {
    ...
    // std::cout << "insert back : " << free_blocks.size() << std::endl;
    // block->Print();
    free_blocks.emplace(block->size_, block);
    // std::cout << "free_blocks size : " << free_blocks.size() << std::endl;
    return true;
  } else {
    std::cout << "unknown addr : " << addr << std::endl;
