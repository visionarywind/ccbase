...
// Use raw ptr to improve performance of memory pool.
using DynamicMemBufRawPtr = struct DynamicMemBuf *;
// Multimap key is the tensor size, for finding the idle memory buf by tensor size.
using SizeMapMemBuf = std::multimap<size_t, DynamicMemBufRawPtr>;
// Map key is the device address, for finding the used memory buf in memory block by device address.
using DeviceAddrMapMemBuf = std::map<DeviceMemPtr, DynamicMemBufRawPtr, DeviceAddrCmp>;
...
class DynamicMemPoolBestFitOpt {
 ...
 protected:
  ...
  virtual size_t AlignMemorySize(size_t size) const {
    return size != 0 ? (size + kDynamicMemAlignSize - 1) & ~(kDynamicMemAlignSize - 1) : kDynamicMemAlignSize;
  }
 ...
 private:
  ...
  inline bool IsSplit(size_t tensor_size, size_t mem_buf_size) const {
    return mem_buf_size - tensor_size >= min_tensor_size_;
  }
  // Split the memory buf by alloc size.
  void SplitMemBuf(size_t size, const DynamicMemBufRawPtr mem_buf, const MemStatusManagerPtr &mem_mng,
                   uint32_t stream_id);
  ...
  void CombineMemBufForward(DeviceAddrMapMemBuf::iterator const &iter, const DynamicMemBlockPtr &mem_block,
                            DynamicMemBufRawPtr mem_buf, const MemStatusManagerPtr &mem_mng,
                            DynamicMemBufStatus target_status);
  ...
  std::tuple<DynamicMemBlockPtr, DynamicMemBufRawPtr, MemStatusManagerPtr> FindByKeepAddr(
    const DeviceMemPtr &device_addr) const;
  DynamicMemBufRawPtr FindMemBufByKeepAddr(const DeviceMemPtr &device_addr, const DynamicMemBlockPtr &mem_block) const;
...
struct DynamicMemBuf {
  DynamicMemBuf(DeviceMemPtr addr, const DynamicMemBlockPtr &mem_block, DynamicMemBufStatus status, size_t size)
      : device_addr_(addr), mem_block_ptr_(mem_block.get()), status_(status), size_(size) {}
  DynamicMemBuf(DeviceMemPtr addr, DynamicMemBlock *mem_block, DynamicMemBufStatus status, size_t size)
      : device_addr_(addr), mem_block_ptr_(mem_block), status_(status), size_(size) {}
  DynamicMemBuf(DeviceMemPtr addr, const DynamicMemBlockPtr &mem_block, DynamicMemBufStatus status, size_t size,
                const std::string &allocator_name, AllocatorType allocator_type)
      : device_addr_(addr),
        mem_block_ptr_(mem_block.get()),
        status_(status),
        size_(size),
        allocator_name_(allocator_name),
        allocator_type_{allocator_type} {}
  DeviceMemPtr device_addr_;
  DynamicMemBlock *mem_block_ptr_;
...
class DynamicMemBlock {
 public:
  ...
  ~DynamicMemBlock() {
    for (auto &entry : block_all_mem_buf_map_) {
      delete entry.second;
    }
    block_all_mem_buf_map_.clear();
  }
...
struct MemStatusManager {
  ...
  bool RemoveDeviceMem(std::vector<SizeMapMemBuf> *container, const size_t size, const DeviceMemPtr &device_addr,
                       uint32_t stream_id);
  void Clear() noexcept;
  ...
  std::vector<std::vector<DynamicMemBlockPtr>> mem_blocks_{decltype(mem_blocks_)(128)};
  std::vector<SizeMapMemBuf> idle_mem_bufs_{decltype(idle_mem_bufs_)(128)};
  std::vector<SizeMapMemBuf> eager_free_mem_bufs_{decltype(eager_free_mem_bufs_)(128)};
};
// }  // namespace device
//}  // namespace mindspore
