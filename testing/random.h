...
void GenerateRandoms(std::uint64_t seed, size_t skip, T *buf, size_t size, Args... args) {
  ...
  // std::cout << "buf : " << buf << ", size : " << size << std::endl
  //           << std::flush;
  Generator gen{seed};
  gen.discard(skip);
 ...
 public:
  TruncatedNormal(T a, T b, T mean, T sigma) : lower_(a), upper_(b), mean_(mean), sigma_(sigma) {
    if (sigma <= 0) {
      ...
      throw std::runtime_error("TruncatedNormal: invalid sigma " + std::to_string(sigma));
    } else {
      alpha_ = (a - mean) / sigma;
      beta_ = (b - mean) / sigma;
    }
